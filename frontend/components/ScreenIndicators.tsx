    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg)', direction: isRtl ? 'rtl' : 'ltr' }}>
      ...
      <div style={{ flex: 1, padding: 'var(--pad-screen)', display: 'flex', flexDirection: 'column', gap: 'var(--gap-grid)', minHeight: 0 }}>
        ...
        <div className="bi-card" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflowX: isMobile ? 'auto' : 'visible' }}>
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: isRtl ? '130px 1fr 180px' : '180px 1fr 130px', gap: 16, minHeight: 0, minWidth: isMobile ? 640 : undefined }}>

            {/* Row labels (always 1st DOM child — gridColumn flips its visual side for RTL) */}
            ...
            {/* Heatmap grid (always the middle, 1fr column) */}
            ...
            {/* Current values (always last DOM child — gridColumn flips its visual side for RTL) */}
            <div style={{ gridColumn: isRtl ? 1 : 3, display: 'flex', flexDirection: 'column', justifyContent: 'space-around', borderRight: isRtl ? '1px solid var(--hairline)' : 'none', borderLeft: isRtl ? 'none' : '1px solid var(--hairline)', paddingRight: isRtl ? 16 : 0, paddingLeft: isRtl ? 0 : 16 }}>
