...
const DETAILS: Record<string, CategoryDetail> = {
  valuation: {
    ...
    episodes: [
      ...
      { date: 'Mar 2000', event: 'Dot-com Peak', signal: 'Shiller CAPE 44.2 — all-time high at the time', outcome: 'S&P 500 fell 49% over 2000–2002' },
...
export default function ScreenMethodologyDetail({ category, palette, onCyclePalette, onNavigate, onOpenTweaks }: Props) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div style={{ flex: 1, display: 'grid', placeItems: 'center' }}>
          <div className="mono" style={{ color: 'var(--ink-4)', fontSize: 12, letterSpacing: '0.1em' }}>UNKNOWN CATEGORY</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg)', paddingBottom: 80 }}>
      ...
      <button
        ...
        style={{
          position: 'fixed', bottom: 28, left: 28, zIndex: 9980,
      ...
      <div style={{ flex: 1, padding: 'var(--pad-screen)', maxWidth: 960, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 'var(--gap-grid)' }}>
        ...
        <motion.div
          ...
          className="bi-card"
          style={{ position: 'relative', paddingLeft: 'calc(var(--pad-card) + 6px)', overflow: 'hidden' }}
        >
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 4, background: 'var(--t-6)', borderRadius: '2px 0 0 2px' }} />

          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 240 }}>
              <div className="bi-eyebrow">METHODOLOGY · INDICATOR</div>
            ...
            <div style={{ display: 'flex', gap: 10, flexShrink: 0, alignItems: 'flex-start' }}>
              <div style={{ padding: '10px 16px', border: '1px solid var(--hairline)', borderRadius: 8, background: 'var(--panel-2)', textAlign: 'center' }}>
                <div className="bi-eyebrow" style={{ fontSize: 9 }}>COMPOSITE WEIGHT</div>
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--gap-grid)' }}
        >
          <motion.div variants={itemVariants} className="bi-card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="bi-card-title">WHAT WE MEASURE</div>
          ...
          <motion.div variants={itemVariants} className="bi-card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="bi-card-title">WHY IT MATTERS FOR BUBBLE DETECTION</div>
            <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.65, textWrap: 'pretty' } as React.CSSProperties}>
              {detail.why_matters}
            </div>
            <div style={{ marginTop: 4, padding: '10px 14px', borderRadius: 8, background: 'var(--panel-2)', borderLeft: '3px solid var(--t-5)' }}>
        ...
        <motion.div variants={itemVariants} initial="hidden" animate="show">
          <div className="bi-card" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div className="bi-card-title">KEY METRICS & THRESHOLDS</div>
              ...
              {detail.metrics.map((metric, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingTop: i > 0 ? 20 : 0, borderTop: i > 0 ? '1px solid var(--hairline)' : 'none' }}>
                  <div>
                    ...
                    <div className="mono" style={{ fontSize: 9.5, color: 'var(--ink-5)', marginTop: 6, letterSpacing: '0.05em' }}>SOURCE: {metric.source}</div>
                  </div>

                  {/* Threshold bar */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {metric.thresholds.map((t, j) => (
                      <div key={j} style={{ display: 'grid', gridTemplateColumns: '28px 130px 1fr auto', gap: 10, alignItems: 'center' }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: LEVEL_COLORS[t.level] ?? 'var(--ink-4)', flexShrink: 0, justifySelf: 'center' }} />
                        <div style={{ fontSize: 11.5, color: 'var(--ink-2)', fontWeight: 500 }}>{t.label}</div>
                        <div style={{ height: 3, borderRadius: 2, background: `linear-gradient(to right, ${LEVEL_COLORS[t.level] ?? 'var(--ink-4)'}, transparent)`, opacity: 0.5 }} />
                        <div className="mono tnum" style={{ fontSize: 11, color: LEVEL_COLORS[t.level] ?? 'var(--ink-4)', fontWeight: 600, whiteSpace: 'nowrap', textAlign: 'right' }}>{t.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        >
          <div className="bi-card-title">HISTORICAL EPISODES</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {detail.episodes.map((ep, i) => (
              <div key={i} style={{
                ...
                <div>
                  <div className="bi-eyebrow" style={{ fontSize: 8.5, marginBottom: 4 }}>SIGNAL</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-3)', lineHeight: 1.5 }}>{ep.signal}</div>
                </div>
                <div>
                  <div className="bi-eyebrow" style={{ fontSize: 8.5, marginBottom: 4 }}>OUTCOME</div>
        ...
        <motion.div
          ...
          style={{ display: 'flex', flexDirection: 'column', gap: 16 }}
        >
          <div className="bi-card-title">HOW TO INTERPRET THE SCORE</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
            {detail.interpretation.map((tier, i) => (
              <div key={i} style={{
                padding: '14px 16px', borderRadius: 8,
                border: '1px solid var(--hairline)',
                background: 'var(--panel-2)',
                borderLeft: `3px solid ${LEVEL_COLORS[tier.level] ?? 'var(--ink-4)'}`,
        ...
        >
          <div className="bi-card-title">ACADEMIC & INDUSTRY SOURCES</div>
            ...
            {detail.sources.map((src, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'baseline', padding: '6px 0', borderTop: i > 0 ? '1px solid var(--hairline)' : 'none' }}>
                ...
                <div>
                  ...
                  <span className="mono" style={{ fontSize: 11, color: 'var(--ink-5)', marginLeft: 8 }}>{src.year}</span>
                </div>
              </div>
            ))}
          </div>
